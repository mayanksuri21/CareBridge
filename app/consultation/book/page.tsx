...
function buildSlotDate(
  date: string | null,
  slot: { hour: number; minute: number; durationMin: number } | null,
): {
  start: Date;
  end: Date;
} | null {
...
export default function BookConsultationPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const bookedLabels = (slots || []).map((slot: any) => {
        ...
        setBookedSlotsForDate(bookedLabels);
      } catch (error) {
        console.error("Booked slot lookup failed:", error);
        if (!cancelled) setBookedSlotsForDate([]);
      }
    })();
  ...
  const activeSlotsForSelectedDoctor = useMemo(() => {
    ...
    if (!selectedDate) {
      if (
        ...
        return selectedDoctorData.active_slots.map((label) =>
          parseTimeString(label),
        );
      }
      return [];
    }
  ...
  const noSlotsConfigured =
    !!selectedDate && activeSlotsForSelectedDoctor.length === 0;
            ...
            <>
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Choose a verified doctor, a preferred date, and one of the
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="rounded-xl border bg-muted/30 p-4 md:col-span-2">
                      <p className="text-xs text-muted-foreground mb-2">
                        {selectedDoctor
                          ? `Available slots for ${selectedDoctorData?.name ?? "selected doctor"}`
                      ...
                      {isLeave ? (
                        <div className="flex items-center gap-2 rounded-lg border border-warning/30 bg-warning/5 p-4 text-sm text-warning-foreground text-amber-700 dark:text-amber-300 font-medium">
                          <span>
                            ⚠️{" "}
                            {noSlotsMessage ||
                              `Dr. ${selectedDoctorData?.name ?? "selected doctor"} is on leave on this date. Please pick another date.`}
                          </span>
                        </div>
                      ) : noSlotsConfigured ? (
                        <div className="flex items-center gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-sm text-amber-700 dark:text-amber-300">
                          ...
                          <span>
                            Doctor has no available consultation hours on this
                            date.
                          </span>
                        </div>
                      ) : dateSlotsLoading ? (
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          activeSlotsForSelectedDoctor.length > 0 ? (
                            activeSlotsForSelectedDoctor.map((s, idx) => {
                              ...
                              if (isDisabled) {
                                chipClass +=
                                  "opacity-40 cursor-not-allowed bg-slate-900/50 text-slate-500 border-slate-800 line-through decoration-slate-600";
                              } else if (active) {
                                chipClass +=
                                  "bg-emerald-600 text-white border-emerald-500 ring-2 ring-emerald-400 font-semibold";
                              } else {
                                chipClass +=
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Review your details and describe what&apos;s bothering you.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                      ...
                      Request saved with status &quot;pending&quot; — doctor
